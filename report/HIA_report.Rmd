...
## Nursery site
Measurement campaigns were completed at 23 wholesale tree production nurseries across each of Australia's continental tree production market regions (Figure 1.). At each nursery batches of tree stock that were currently ready for sale were identified with nursery production managers. From these batches, tree stock in containers &ge; 18 l were selected for measurements. For each nursery, the 30yr mean annual temperature (MAT) and precipitation (MAP) were downloaded for each site at 1km^2 resolution (http://www.worldclim.org/; Table 1).

## Visual quality assessments
Prior to data collection (explained below), an above and bewloground visual assessment of moprhological quality was completed for each selected batch of tree stock. This methodology ensures that data collection was representative of trees possessed the quality morphological attributes required by AS2303 at dispatch. First, 


Next, 

(2)	Complete visual quality assessments of above and belowground morphology (AS2303).
aboveground structural support and form, free of pests, injury and disease
(3) below
sufficient rootball size, no root defects


## Aboveground allometric parameters

Height and diameter at 30cm were measured on a large subset of trees for each selected batch of tree stock. Up to 45 trees were measured for containers	&le; 45 L and up to 20 trees were measured for all larger sized containers, if available. For each measured tree the "Size Index" parameter was calculated as the product of height and diameter.

## Leaf and canopy traits
Parameters related to canopy shape and form were measured for a subset of 10 trees per batch, if available. Maximum branch length for the trunk was recorded for the longest visible branch which was then doubled to estimate maximum canopy spread for each tree. Canopy length was measured from the initiation point of the lowest branch to the total height of the tree. A branchiness parameter (**b**_t) was calculated as the number of branches in a 30cm interval from the initiation point of the lowest branch. 

\
Leaf area and dry mass were recorded for 3 fully developed full sun leaves for each of the 10 trees, when possible. Leaves were not sampled for deciduous trees in winter or during bud-out. Additionally, some trees in large size containers  were too tall to sample leaves. 

##Data analysis
Differences in measured parameters with container volume were analysed by mixed-effects models in R with individual tree species and nurseries as random effects. Nurseries were also considered as a fixed effect in some models to explain the variance of different nursery practives. Mixed model analyses were perfromed with the 'lme4' package in R. Explained variance (*R*^2^) of mixed models were computed as in @nakagawa2013general, in which the marginal *R*^2^ represents variance explained by fixed factors and the conditional *R*^2^ by both fixed and random factors.
