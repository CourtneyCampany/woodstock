...
plot_si_ranges <- function(fits, whichfits=1:length(fits), xlim, ylim, labsrt=30,
  ...
  text(xat, y, labs, pos=2, font=2, cex=0.8, srt=labsrt)
  
  # xat <- u[1] + 0.5*(u[2] - u[1])
  # y <- sapply(fits, function(x)predict(x, newdata=data.frame(volume = 10^xat)))
  # y <- y[1:(length(y)-1)] + diff(y)/2
  #hmmmm labs <- 
  #text(xat, y, labs, pos=1, font=2, cex=0.8, srt=labsrt)
...
```{r}
plot_si_ranges(fit_small_ever, xlim=log10(c(18,100)), ylim=log10(c(8,200)), labsrt=35)
```

```{r}
plot_si_ranges(fit_small_ever, whichfits=c(1,3,4,6), 
               poly_colors=c("lightgrey","grey","lightgrey"),
               quantiles=c(0.05,0.25,0.75,0.95),
               xlim=log10(c(18,100)), ylim=log10(c(8,200)), labsrt=35)
```

# Small, deciduous

```{r}
plot_si_ranges(fit_small_deci, xlim=log10(c(18,100)), ylim=log10(c(8,200)))
```

```{r}
plot_si_ranges(fit_small_deci, 
               whichfits=c(1,3,4,6), 
               poly_colors=c("lightgrey","grey","lightgrey"),
               quantiles=c(0.05,0.25,0.75,0.95),
...
```{r}
plot_si_ranges(fit_large, xlim=log10(c(100,3000)), ylim=log10(c(50,3000)))
```

```{r}
plot_si_ranges(fit_large, 
               whichfits=c(1,3,4,6), 
               poly_colors=c("lightgrey","grey","lightgrey"),
               quantiles=c(0.05,0.25,0.75,0.95),
