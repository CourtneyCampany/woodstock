...
fits <- lapply(quantiles, function(x)rq(log10(si) ~ log10(volume), data=sitree_small_ever, tau=x))
...
```{r}
quantiles <- c(0.05,0.25,0.75,0.95)
fits <- lapply(quantiles, function(x)rq(log10(si) ~ log10(volume), data=sitree_small_deci, tau=x))

plot_si_ranges(fits, 
               poly_colors=c("lightgrey","grey","lightgrey"),
               quantiles=quantiles, labyadj=-0.04, 
               xlim=log10(c(18,100)), ylim=log10(c(8,200)), labsrt=22)
```
