...
sizeindex$color <- ifelse(sizeindex$climate_region == "South Australia", alpha("forestgreen", .4), "black")
sizeindex$color <- ifelse(sizeindex$climate_region == "New South Wales", alpha("goldenrod1",.4), sizeindex$color)
sizeindex$color <- ifelse(sizeindex$climate_region == "Western Australia", alpha("navyblue", .4), sizeindex$color)
sizeindex$color <- ifelse(sizeindex$climate_region == "Northern Territory", alpha("firebrick2",.4), sizeindex$color)
sizeindex$color <- ifelse(sizeindex$climate_region == "Victoria", alpha("darkorchid3",.4), sizeindex$color)
sizeindex$color <- ifelse(sizeindex$climate_region == "Queensland", alpha("deepskyblue1",.4), sizeindex$color)
...
## Aims: Evalute Root:Shoot balance in nursery trees
<hr>
<br />

<IMG STYLE="position:absolute; TOP:200px; LEFT:25px; WIDTH:500px; HEIGHT:400px" SRC="img/concept.png">
...
library(knitr)
kable(volumetop, format = "html", table.attr= 'class="table table-hover"', align = 'lr')

```

<IMG STYLE="position:absolute; TOP:200px; LEFT:600px; WIDTH:336px; HEIGHT:448px" SRC="img/3000tree.jpg">


## Database of Size Index information
<hr>
<br />
...
shinyApp(
  ...
  plot(log10(sizeindex.mean) ~ log10(volume), data=sizeindex, xlab="Container volume (L)", 
      ...
      points(log10(nursery_ss()[[4]])~ log10(nursery_ss()[[5]]), col=alpha("red", .6), pch=19,cex=1.25)
...
pick_tree<- c("corymbia_maculata", "agonis_flexuosa", "lophostemon_confertus", "angophora_costata", 
...
si_species$treecol <- ifelse(si_species$species == "corymbia_maculata", alpha("forestgreen", .4), 
                        ifelse(si_species$species == "agonis_flexuosa", alpha("goldenrod1",.4),
                        ifelse(si_species$species == "lophostemon_confertus", alpha("navyblue",.4),     
                        ifelse(si_species$species == "angophora_costata", alpha("firebrick2",.4),
                        ifelse(si_species$species == "magnolia_grandiflora_littlegem", alpha("darkorchid3",.4),
                        ifelse(si_species$species == "brachychiton_acerifolius", alpha("deepskyblue1",.4),
                        ifelse(si_species$species == "jacaranda_mimosifolia", alpha("indianred3",.4),
                        ifelse(si_species$species == "araucaria_heterophylla", alpha("yellow2",.4),
                        # ifelse(si_species$species == "ficus_microcarpa_hilliiflash", alpha("cornflowerblue",.4),
                       alpha("yellow2",.4)))))))))
...
shinyApp(
  ...
  server = function(input, output, session) {
    ...
    tree_ss<- reactive({subset(si_species, species %in% input$whichtree)
                      })
  ...
  plot(log10(sizeindex.mean) ~ log10(volume), data=sizeindex, xlab="Container volume (L)", 
      ...
      points(log10(tree_ss()[[4]])~ log10(tree_ss()[[5]]), bg=tree_ss()[[8]], pch=21, cex=2)
...
shinyApp(
  ...
  plot(log10(sizeindex.mean) ~ log10(volume), data=sizeindex, xlab="Container volume (L)", 
      ...
      points(log10(region_ss()[[4]])~ log10(region_ss()[[5]]), bg=region_ss()[,7], pch=21, cex=2)
...
si_taper$formcol <- ifelse(si_taper$thickness == "skinny", alpha("forestgreen", .4), 
                        ifelse(si_taper$thickness == "average", alpha("goldenrod1",.4),
                        alpha("navyblue",.4)))
...
shinyApp(
  ...
  plot(log10(sizeindex.mean) ~ log10(volume), data=sizeindex, xlab="Container volume (L)", 
      ...
      points(log10(taper_ss()[[1]])~ log10(taper_ss()[[2]]), bg=taper_ss()[[7]], pch=21, cex=2)
...
* <strong>Anaylsis: What does data driven Size Index tell us </strong> 
  + When/Where/Why are tree stock too big or too small?
  + Is there an appropriate upper and lower limit?

<br />

* <strong>Is there a means to reduce the variation for look-up tables?</strong>   
